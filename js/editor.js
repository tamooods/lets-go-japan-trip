...
function openPlaceEditor(day, place) {
  ...
  const actsArr =
    place && place.acts
      ? Array.isArray(place.acts)
        ? place.acts
        : JSON.parse(place.acts)
      : [];
  document.getElementById("place-editor-acts").value = actsArr.join("\n");
  document.getElementById("place-editor-expense").value = place
    ? place.expense || 0
    : 0;
...
function renderSplitCheckboxes(place) {
  ...
  if (!window.members || !window.members.length) {
    container.innerHTML =
      '<span style="font-size:0.8rem;color:var(--gray)">ไม่มีสมาชิก</span>';
    return;
  }
...
async function savePlaceEditor() {
  ...
  if (!name) {
    ...
    document
      .getElementById("place-editor-name-error")
  ...
  const acts = document
    ...
    .filter(Boolean);
  const expense =
    parseFloat(document.getElementById("place-editor-expense").value) || 0;
...
async function searchPlaceHandler() {
  ...
  if (!results.length) {
    container.innerHTML =
      '<div class="search-result-item">ไม่พบสถานที่ ลองคลิกบนแผนที่</div>';
  } else {
    results.forEach((r) => {
...
function enablePlacePickMode() {
  ...
  document.getElementById("place-editor-search").placeholder =
...
document
  .getElementById("place-editor-save")
  .addEventListener("click", savePlaceEditor);
document
  .getElementById("place-editor-cancel")
  .addEventListener("click", closePlaceEditor);
document
  .getElementById("place-editor-close")
  .addEventListener("click", closePlaceEditor);
document
